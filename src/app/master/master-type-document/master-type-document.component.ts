...
export class MasterTypeDocumentComponent implements OnInit {
  myForm: FormGroup;
...
updateBrand(brand) {
  ...
  document.getElementById( 'uploadBrand').click();
   ...
   if ( !this.myForm.invalid) {
    ...
    this.restService.createTypeDocument(this.myForm.get('description').value.toUpperCase(), statusTemp).then(data => {
      const resp: any = data;
      console.log(resp);
  ...
  sendUpdateBrand() {
   ...
   if ( !this.myFormUpdate.invalid) {
    ...
    this.restService.updateTypeDocument(Number(this.currentBrand.id),
     this.myFormUpdate.get('descriptionUpdate').value.toUpperCase(), statusTemp)
    .then(data => {
      const resp: any = data;
      console.log(resp);
      if (resp.success === false) {
  ...
  deleteBrand(brand: any) {
    ...
    .then((willDelete) => {
        if (willDelete.value) {
          ...
          swal.showLoading();
          this.restService.deleteTypeDocument(Number(this.elementDelete.id))
          .then(data => {
            swal.showLoading();
            const resp: any = data;
            console.log(resp);
