...
@Component({
  selector: 'app-master-update-estimate-customer',
  templateUrl: './master-update-estimate-customer.component.html',
  styleUrls: ['./master-update-estimate-customer.component.scss',
    '../../../assets/icon/icofont/css/icofont.scss']
})
export class MasterUpdateEstimateCustomerComponent implements OnInit {
  ...
  fileEstimateImagesTemp: Array<FileEstimate> = []; // Este array es para guardar las imagenes organizadas
  fileEstimateImage: FileEstimate;

  fileEstimateTemp: Array<FileEstimate> = []; // Este array es para guardar las imagenes organizadas
  ...
  constructor(private restService: RestService, private router: Router, private estimateService: EstimateService, private excelService: FilexcelService, private workService: WorkService, private uploadService: UploadService, private forkliftService: ForkliftService, private rutaActiva: ActivatedRoute, private resumeneServices: ResumenesService) {
    ...
    this.getEstimateSpecific(this.estimateId);
    this.getEstimateParts();
    this.getEstimateWorkforce();
    this.getFilesEstimateImage();
  ...
  getPendingForklift() {
    this.resumeneServices.getForkliftPendingGeneral(this.selectedForkliftId).then(data => {
      const resp: any = data;
  ...
  createEstimateDetail() {
    ...
    // let priceTemp = this.price.toString().replace('.','').replace(',','.');

    // let priceTempStr = this.price.toString();
    // priceTempStr = priceTempStr.split('.').join('');
    let priceTemp = this.changeFormatDecimal(this.price);// 
    //this.suggestedPrice = priceTemp;
  ...
  updateEstimateDetail() {
    ...
    console.log('priceUpdateTemp');
    console.log(priceUpdateTemp);


    let suggestedP = priceUpdateTemp;
    ...
    if (Number(priceUpdateTemp) >= Number(this.lowPriceUpdate) && Number(priceUpdateTemp) <= Number(this.higherPriceUpdate)) {
      ...
      console.log('Entro if de actulizar');
 ...
 */
  ...
  getConfigEstimatesInitial() {
    this.estimateService.getConfigEstimates().then(data => {
      ...
      console.log('suggestedMaximum');
      console.log('Valor ' + this.suggestedMaximum);
      ...
      console.log('this.configDetailEstimates');
      console.log(this.configDetailEstimates);
  ...
  finalOperationUpdate(country: number) {
    ...
    if (this.conditionValidation == 3) {
      console.log('valor tariff ' + this.managmentTariff);
    ...
    console.log('high');
    console.log(this.higherPriceUpdate);
    console.log(this.lowPriceUpdate + '---manada de boludos---' + this.higherPriceUpdate);
        ...
        if (event.target.files && event.target.files[0]) {
          ...
          for (let i = 0; i < filesAmount; i++) {
            ...
            reader.onload = (event: any) => {
              ...
              this.fileEstimateImage = {
                url: event.target.result,
                file: event.target.files
              };

              this.fileEstimateImagesTemp.push(this.fileEstimateImage);
  ...
  deleteImage(item: any, i: number) {
    ...
    if (item.id) {
      swal({
        ...
        .then((willDelete) => {
          if (willDelete.value) {
            ...
            console.log(item);
            swal.showLoading();
            this.estimateService.deleteEstimateFile(Number(item.id))
              .then(data => {
                swal.showLoading();
                const resp: any = data;
                console.log(resp);
                ...
                if (resp.success === false) {
                  swal({
                    title: 'Esta imagen presenta problemas',
                    text: 'Esta imagen no se puede eliminar',
                    type: 'error'
                  });
                } else {
                  // this.router.navigateByUrl('master/registerBrand');
      ...
      this.fileEstimateImagesTemp.splice(i, 1);

      var j = this.contImages - i;
      console.log('este es valor de la posición de j: ' + j);
      this.selectedFilesImages.splice(j, 1);
    }
  }
  ...
  /* validateEmail( email ) 
 {
  ...
  createEstimateDetailWorkforce() {
    ...
    console.log('Pureba llamada a metodo; mandar a servicio, subtotal es: ' + subtotalTemp);
  ...
  calculateSubtotalUpdate() {
    ...
    priceUpdateTemp = priceUpdateTemp.toString().replace('.', '').replace('.', '').replace('.', '').replace('.', '').replace(',', '.');//this.finalFormatStandard(priceUpdateTemp);
  ...
  ///Metodo que hace el verdadero cambio en los valores
  ...
  finalFormatPriceUpdate() {
    ...
    console.log('price---- ' + this.priceUpdate);
    console.log(splitLeft);
  ...
  getFilesEstimateImage() {
    ...
    this.estimateService.getEstimateDetailFilesImages(Number(this.estimateId)).then(data => {
      ...
      for (let EstimateImage of resp.data) {
        //console.log(EstimateImage.name);
        console.log('Jajajajaja');

        this.fileEstimateImage = {
          id: EstimateImage.id,
          url: EstimateImage.url,
          content: '',
          type: EstimateImage.type
        };

        this.fileEstimateImagesTemp.push(this.fileEstimateImage);
        this.urlsImages.push(EstimateImage.url);
      ...
      this.contImages = this.urlsImages.length;
