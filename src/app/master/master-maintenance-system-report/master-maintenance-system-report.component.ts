...
@Component({
  selector: "app-master-maintenance-system-report",
  templateUrl: "./master-maintenance-system-report.component.html",
  styleUrls: [
    "./master-maintenance-system-report.component.scss",
  ...
  providers: [
    I18n,
    {
      ...
      useClass: MasterMaintenanceSystemReportComponent,
    },
  ],
})
export class MasterMaintenanceSystemReportComponent extends NgbDatepickerI18n {
  ...
  getFilters() {
    ...
    if (this.selectedRegionalId == 0) {
      ...
      if (cont >= 1) {
        ...
        console.log(".---------->" + params);
        this.reportService
          .showSystemMauntenanceMa(params)
          .then((data) => {
            const resp: any = data;
            ...
            this.exportAsExcelFile(
              this.dataExcels,
              "Informe de Sistemas Intervenidos " + fromD + " - " + untilD
            );
            swal.close();
  ...
  public exportAsExcelFile(rows: any[], excelFileName: string): void {
    if (rows.length > 0) {
      ...
      const workbook: XLSX.WorkBook = {
        Sheets: { "Info-Sis-Intervenidos": worksheet },
        SheetNames: ["Info-Sis-Intervenidos"],
      };
